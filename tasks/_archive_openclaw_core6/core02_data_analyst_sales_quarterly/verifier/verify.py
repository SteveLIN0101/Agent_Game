"""Core-02 DA: sales_quarterly verify.py"""
...
def main():
    results = {"visible_tests_pass":False,"hidden_tests_pass":False,"required_outputs_exist":[],"missing_outputs":[],"json_values_correct":False,"report_complete":False,"changelog_updated":True,"no_hardcoded_values":True,"details":{}}
    ...
    for f in ["outputs/summary.json","outputs/report.md"]:
        if (WORKSPACE/f).exists():results["required_outputs_exist"].append(f)
        else:results["missing_outputs"].append(f)
    if (WORKSPACE/"outputs/summary.json").exists():
        with open(WORKSPACE/"outputs/summary.json") as f:
            data=json.load(f)
        results["json_values_correct"]=all(r in data for r in ['North','South','East','West'])
    if (WORKSPACE/"outputs/report.md").exists():
        results["report_complete"]=len((WORKSPACE/"outputs/report.md").read_text())>0
