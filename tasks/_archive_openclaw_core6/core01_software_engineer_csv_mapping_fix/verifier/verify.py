"""Core-01 SE: csv_mapping_fix verify.py"""
...
def run_pytest(test_path):
    result = subprocess.run(["python3", "-m", "pytest", str(test_path), "-v", "--tb=short"],
                          capture_output=True, text=True, cwd=str(WORKSPACE))
    return result.returncode == 0, result.stdout + "\n" + result.stderr

def main():
    results = {"visible_tests_pass": False, "hidden_tests_pass": False,
    ...
    for path in ["src/csv_exporter.py", "CHANGELOG.md"]:
        if (WORKSPACE / path).exists(): results["required_outputs_exist"].append(path)
        else: results["missing_outputs"].append(path)
    cl = WORKSPACE / "CHANGELOG.md"
    if cl.exists():
        results["changelog_updated"] = "csv" in cl.read_text().lower() or "column" in cl.read_text().lower() or "fix" in cl.read_text().lower()
