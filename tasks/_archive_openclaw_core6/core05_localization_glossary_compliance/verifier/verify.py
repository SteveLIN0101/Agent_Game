...
def main():
    ...
    for f in ["output/strings_zh.json", "output/localization_qa.json"]:
    ...
    if (WORKSPACE/"output/strings_zh.json").exists():
        tgt_text=json.dumps(json.loads((WORKSPACE/"output/strings_zh.json").read_text()),ensure_ascii=False)
        results["glossary_compliance"]='空间站' not in tgt_text and '开票系统' not in tgt_text and '模版' not in tgt_text
