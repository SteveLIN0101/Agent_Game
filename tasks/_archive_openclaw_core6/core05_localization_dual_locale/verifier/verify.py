...
def main():
    ...
    for f in ["output/strings_zh.json", "output/strings_ja.json", "output/localization_qa.json"]:
    ...
    if (WORKSPACE/"output/strings_zh.json").exists() and (WORKSPACE/"source/strings_en.json").exists():
        ...
        tgt_zh=json.loads((WORKSPACE/"output/strings_zh.json").read_text())
        tgt_ja=json.loads((WORKSPACE/"output/strings_ja.json").read_text())
        results["key_integrity"]=set(src.keys())==set(tgt_zh.keys())==set(tgt_ja.keys())
