"""Core-05 Localization verifier.

Checks: key integrity, placeholder preservation, glossary compliance,
length limits, QA report completeness.
"""
...
class LocalizationVerifier(AbstractVerifier):
    """Verifier for localization tasks (Core-05)."""
    ...
    async def check_constraints(self, session: SessionState) -> ConstraintResult:
        ...
        stdout, _, _ = self._exec(
            ...
            "src=json.load(open('/workspace/source/strings_en.json')); "
            "tgt=json.load(open('/workspace/output/strings_zh.json')); "
            "print('OK' if set(src.keys())==set(tgt.keys()) else 'KEY_MISMATCH')\""
        )
        if "KEY_MISMATCH" in stdout:
            violations.append("key_integrity_failed")

        stdout, _, _ = self._exec(
            session, "cat /workspace/output/strings_zh.json"
        )
        forbidden = ["空间站", "开票系统", "外挂", "模版"]
        for term in forbidden:
            if term in stdout:
                violations.append(f"forbidden_translation: {term}")
